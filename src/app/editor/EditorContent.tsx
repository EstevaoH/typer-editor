...
const StatisticsDialog = lazy(() =>
  import("@/components/statistics-dialog").then((module) => ({
    default: module.StatisticsDialog,
  }))
);
const DocumentBreadcrumb = lazy(() =>
  import("@/components/document-breadcrumb").then((module) => ({
    default: module.DocumentBreadcrumb,
  }))
);
  ...
  const handleToggleFavorite = useCallback(
    (id: string) => {
      if (currentDocument) {
        toggleFavorite(id);
        toast.showToast(
          currentDocument.isFavorite ? "⭐ Desfavoritado" : "🌟 Favoritado"
        );
      }
    },
                ...
                <div className="flex items-center gap-3">
                  ...
                  {!session?.user && (
                    <div
                      ...
                      title="Status de salvamento local"
                    >
                  ...
                  {session?.user ? (
                    <>
                      ...
                      {documentStatus.hasUnsavedChanges && (
                        <div
                          className="flex items-center gap-1.5"
                          title="Há mudanças não sincronizadas com a nuvem"
                        >
                          <div className="flex items-center gap-1 text-yellow-400">
